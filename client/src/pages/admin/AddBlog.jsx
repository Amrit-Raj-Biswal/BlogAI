...
const AddBlog = () => {
  ...
  const generateContent = async () => {
    if(!title) return toast.error('Enter a title to generate response')
      try {
        setLoading(true)
      ...
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
        ...
        <div className='max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative'>
          <div ref={editorRef}></div>
            {loading && ( <div className='absolute right-0 top-0 bottom-0 l0 left-0 flex items-center justify-center bg-black/10 mt-2'>
        ...
        <button disabled = {isAdding} type="submit" className='mt-8 w-40 h-10 bg-blue-600 text-white rounded cursor-pointer text-sm'>{isAdding ? 'Adding...' : 'Add Blog'}</button>     
